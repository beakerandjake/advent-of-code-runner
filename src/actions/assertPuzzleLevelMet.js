...
export const assertPuzzleLevelMet = async ({ year, day, level } = {}) => {
  if (!await requiredPartsHaveBeenSolved(year, day, level)) {
    logger.error(`You cannot attempt this puzzle (day ${day}, level ${level}) because you have not completed the previous levels of this puzzle.`);
    return false;
  }
  return true;
};
