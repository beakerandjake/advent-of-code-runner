...
const { deleteExistingInputFiles } = await import('../../src/initialize/deleteExistingInputFiles.js');
...
describe('initialize', () => {
  describe('deleteExistingInputFiles()', () => {
    ...
    test('empties inputsDir', async () => {
      const expected = 'ASDF';
      getConfigValue.mockImplementation((key) => (key === 'paths.inputsDir' ? expected : undefined));
