...
describe('initialize', () => {
  describe('createDataFile()', () => {
    ...
    test('loads template data file', async () => {
      ...
      getConfigValue.mockImplementation((key) => (key === 'paths.templates.userDataFile' ? paths : undefined));
    ...
    test('tokenizes template file contents', async () => {
      getConfigValue.mockImplementation((key) => (key === 'paths.templates.userDataFile' ? {} : undefined));
