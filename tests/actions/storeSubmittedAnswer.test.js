...
const { addIncorrectAnswer, setCorrectAnswer } = await import(
  '../../src/answers.js'
);
const { storeSubmittedAnswer } = await import(
  '../../src/actions/storeSubmittedAnswer.js'
);
...
describe('storeSubmittedAnswer()', () => {
  ...
  test.each([null, undefined])(
    'throws if submissionResult is %s',
  ...
  test.each([null, undefined])(
    'throws if submissionResult.correct is %s',
    async (correct) => {
      await expect(async () =>
        storeSubmittedAnswer({
          year: 2022,
